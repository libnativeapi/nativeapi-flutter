...
class SecureStorage {
  ...
  static SecureStorage? createWithScope(String scope) {
    ...
    final handle = c.native_secure_storage_create_with_scope(scopeNative);
  ...
  bool set(String key, String value) {
    ...
    final result = c.native_secure_storage_set(
      nativeHandle,
      keyNative,
      valueNative,
    );
  ...
  String? get(String key, String defaultValue) {
    ...
    final resultPointer = c.native_secure_storage_get(
      nativeHandle,
      keyNative,
      defaultValueNative,
    );
  ...
  bool remove(String key) {
    final keyNative = key.toNativeUtf8().cast<ffi.Char>();
    final result = c.native_secure_storage_remove(nativeHandle, keyNative);
    pkg_ffi.calloc.free(keyNative);
    return result;
  }

  bool clear() {
    return c.native_secure_storage_clear(nativeHandle);
  }

  bool contains(String key) {
    final keyNative = key.toNativeUtf8().cast<ffi.Char>();
    final result = c.native_secure_storage_contains(nativeHandle, keyNative);
    pkg_ffi.calloc.free(keyNative);
    return result;
  }

  List<String> get keys {
    final list = c.native_secure_storage_get_keys(nativeHandle);
    final items = <String>[];
  ...
  int get size {
    return c.native_secure_storage_get_size(nativeHandle);
  }

  Map<String, String> get all {
    final raw = c.native_secure_storage_get_all(nativeHandle);
    ...
    rawPointer.ref = raw;
    c.native_string_map_free(rawPointer);
    pkg_ffi.calloc.free(rawPointer);
    return entries;
  }

  String? get scope {
    final resultPointer = c.native_secure_storage_get_scope(nativeHandle);
    if (resultPointer == ffi.nullptr) return null;
    final result = resultPointer.cast<pkg_ffi.Utf8>().toDartString();
    c.free_c_str(resultPointer);
    return result;
  ...
  static bool isAvailable() {
    return c.native_secure_storage_is_available();
  }
}
