...
class Image {
  ...
  static Image? fromBase64(String base64Data) {
    ...
    final handle = c.native_image_from_base64(base64DataNative);
  ...
  Size get size {
    final raw = c.native_image_get_size(nativeHandle);
    return Size.fromNative(raw);
  }

  String? get format {
    final resultPointer = c.native_image_get_format(nativeHandle);
  ...
  String? toBase64() {
    final resultPointer = c.native_image_to_base64(nativeHandle);
    if (resultPointer == ffi.nullptr) return null;
    final result = resultPointer.cast<pkg_ffi.Utf8>().toDartString();
    c.free_c_str(resultPointer);
    return result;
  ...
  bool saveToFile(String filePath) {
    final filePathNative = filePath.toNativeUtf8().cast<ffi.Char>();
    final result = c.native_image_save_to_file(nativeHandle, filePathNative);
    pkg_ffi.calloc.free(filePathNative);
    return result;
  }
  ...
  ffi.Pointer<ffi.Void> get nativeObject =>
      c.native_image_get_native_object(nativeHandle);
}
