...
class Preferences {
  ...
  static Preferences? createWithScope(String scope) {
    ...
    final handle = c.native_preferences_create_with_scope(scopeNative);
  ...
  bool set(String key, String value) {
    ...
    final result = c.native_preferences_set(
      nativeHandle,
      keyNative,
      valueNative,
    );
  ...
  String? get(String key, String defaultValue) {
    ...
    final resultPointer = c.native_preferences_get(
      nativeHandle,
      keyNative,
      defaultValueNative,
    );
  ...
  bool remove(String key) {
    final keyNative = key.toNativeUtf8().cast<ffi.Char>();
    final result = c.native_preferences_remove(nativeHandle, keyNative);
    pkg_ffi.calloc.free(keyNative);
    return result;
  }

  bool clear() {
    return c.native_preferences_clear(nativeHandle);
  }

  bool contains(String key) {
    final keyNative = key.toNativeUtf8().cast<ffi.Char>();
    final result = c.native_preferences_contains(nativeHandle, keyNative);
    pkg_ffi.calloc.free(keyNative);
    return result;
  }

  List<String> get keys {
    final list = c.native_preferences_get_keys(nativeHandle);
    final items = <String>[];
  ...
  int get size {
    return c.native_preferences_get_size(nativeHandle);
  }

  Map<String, String> get all {
    final raw = c.native_preferences_get_all(nativeHandle);
  ...
  String? get scope {
    final resultPointer = c.native_preferences_get_scope(nativeHandle);
    if (resultPointer == ffi.nullptr) return null;
    final result = resultPointer.cast<pkg_ffi.Utf8>().toDartString();
    c.free_c_str(resultPointer);
    return result;
