...
import 'package:nativeapi/src/drop_target.dart';
import 'package:nativeapi/src/support.dart';
  ...
  void _handle(DropTargetEvent event) {
    switch (event) {
      case DropTargetEnteredEvent(:final position):
        _current = null;
        _moveTo(_regionAt(position), position);
      ...
      case DropTargetDroppedEvent(
        ...
        region?.widget.onDropped?.call(
          DropRegionDropDetails(
            localPosition: region._local(position),
            filePaths: filePaths,
            text: text,
          ),
        );
