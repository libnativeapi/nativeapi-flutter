#!/usr/bin/env python3
"""
Script to update include statements in cnativeapi.mm based on available source files.
This script automatically scans for C++ and Objective-C++ source files and updates
the include statements in the cnativeapi.mm file.
"""
...
def find_source_files(src_dir):
    ...
    for file_path in source_files:
        ...
        # Convert to relative path from cnativeapi.mm location
        rel_path = os.path.relpath(
            file_path, Path(__file__).parent / "packages/cnativeapi/macos/Classes"
        )
...
def update_nativeapi_mm(nativeapi_path, source_files):
    """Update the cnativeapi.mm file with new include statements."""
    ...
    if not match:
        print("Could not find '// Include source files' section in cnativeapi.mm")
        return False
    ...
    new_content = (
        content[: match.start()] + new_include_section + content[match.end() - 1 :]
    )
...
def main():
    ...
    nativeapi_path = script_dir / "packages/cnativeapi/macos/Classes/cnativeapi.mm"
    ...
    if not nativeapi_path.exists():
        print(f"cnativeapi.mm file not found: {nativeapi_path}")
        return 1
    ...
    # Update cnativeapi.mm
    print(f"\nUpdating {nativeapi_path}...")
    if update_nativeapi_mm(nativeapi_path, source_files):
        print("Successfully updated cnativeapi.mm!")
        return 0
    else:
        print("Failed to update cnativeapi.mm")
        return 1
